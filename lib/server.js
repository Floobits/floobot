...
var Server = function () {
  ...
  self.app.post("/deploy/:project/:env", self.handle_deploy);
...
Server.prototype.handle_deploy = function (req, res) {
  ...
  msg = util.format("%s deployed %s to %s", req.username, req.params.project, req.params.env);

  debugger;
  _.each(self.irc_client.chans, function (channel) {
    self.irc_client.say(channel, msg);
  });
