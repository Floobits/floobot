...
Server.prototype.on_msg = function (from, to, msg) {
  ...
  _.each(urls, function (msg_url) {
    ...
    request.get(msg_url, function (err, response, body) {
      var $,
        username,
        title,
        retweets,
      ...
      if (response.statusCode >= 400) {
        ...
        if (parsed_url.hostname === "twitter.com") {
          username = $("#page-container > div.permalink.light-inline-actions.stream-uncapped.has-replies > div.permalink-inner.permalink-tweet-container > div > div > div.content.clearfix > div > a > span.username.js-action-profile-name > b");
          ...
          title = util.format("<@%s> %s (%s retweets, %s favorites)", username, title, retweets, favorites);
        } else if (parsed_url.hostname === "youtube.com") {
